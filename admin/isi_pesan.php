...
$data_user_1 = $db_connect->query("SELECT * FROM user WHERE id = '$userID1'")->fetch_assoc();
$result = $db_connect->query($query);
$photo_user = $data_user_1['photo'];
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if (!$isPengirim) {
        ...
        echo '<img alt="Pic" src="../image/user_profile/' . $photo_user . '">';
        echo '</div>';
    }
    ...
    echo '<a href="#" class="fs-5 fw-bold text-gray-900 text-hover-primary ' . ($isPengirim ? 'ms-1' : 'me-1') . '">' . ($isPengirim ? 'You' : $data_user_1['nama']) . '</a>';
