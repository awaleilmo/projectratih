...
if (isset($_GET['setuju'])) {
    $idDetailPenjualan = $_GET['setuju'];
    if (accept_reschedule($idDetailPenjualan)) {
        // echo "<script>alert('berhasil');location.href='reschedule'</script>";
    }
}

// Mendapatkan nilai 'tolak' dari parameter URL
if (isset($_GET['tolak'])) {
    $idDetailPenjualan = $_GET['tolak'];
    if (tolak_reschedule($idDetailPenjualan)) {
        echo "<script>alert('berhasil');location.href='reschedule'</script>";
    }
}

$data_pengajuan = $db_connect->query("SELECT detail_penjualan.id AS id_detail_penjualan, produk.nama, reschedule.tgl_asal, reschedule.tgl_pengajuan, reschedule.keterangan, detail_penjualan.nomor_invoice, reschedule.status FROM reschedule JOIN detail_penjualan ON reschedule.detail_penjualan = detail_penjualan.id JOIN produk ON detail_penjualan.id_produk = produk.id;
");
?>
<!DOCTYPE html>
<!--
...
<head>
    <base href="" />
    <title>Wld Project | Pengajuan Reschedule</title>
    <meta charset="utf-8" />
                        ...
                        <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
                            ...
                            <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
                                ...
                                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                                    <!--begin::Title-->
                                    <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Reschedule</h1>
                                    <!--end::Title-->
                                    <!--begin::Breadcrumb-->
                                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                                        ...
                                        <!--end::Item-->
                                        <!--begin::Item-->
                                        <li class="breadcrumb-item text-muted">List Reschedule</li>
                                        <!--end::Item-->
                                    </ul>
                                    <!--end::Breadcrumb-->
                                </div>
                                            ...
                                            <table class="table align-middle table-row-dashed fs-6 gy-5 mb-0">
                                                ...
                                                <tbody class="fw-semibold ">
                                                    <?php foreach ($data_pengajuan as $data) : ?>
                                                        <tr>
                                                            <td><a href="info_pesanan?pesanan=<?= $data['nomor_invoice'] ?>"><?= $data['nomor_invoice'] ?></a></td>
                                                            <td><?= $data['nama'] ?></td>
                                                            <td><?= $data['tgl_asal'] ?></td>
